...
const isValidSession = (s: unknown): s is PrayerSession => {
  return (
    typeof s === "object" &&
    s !== null &&
...
const readHistory = (): PrayerSession[] => {
  try {
...
export const recordCompletion = (mysteryKey: MysteryKey): void => {
...
const computeLastPrayedDaysAgo = (sessions: PrayerSession[]): number | null => {
...
const computeStreak = (sessions: PrayerSession[]): number => {
  if (sessions.length === 0) return 0;

  const prayedDates = new Set<string>(
    sessions.map((s) => toLocalDateKey(s.completedAt))
  );
...
export const usePrayerHistory = (): PrayerHistoryState => {
