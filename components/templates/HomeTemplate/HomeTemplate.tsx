...
import { Play, BookOpen, Sun, Moon, Settings, Heart } from "lucide-react";
...
export function HomeTemplate({ locale, todaysMystery }: HomeTemplateProps) {
  ...
  const dateStr = useMemo(() => {
    const now = new Date();
    const monthNames = locale === "la" ? MONTH_NAMES_LA : MONTH_NAMES_PT;
    const dayNames = locale === "la" ? DAY_NAMES_LA : DAY_NAMES_PT;
  ...
  const todayMysteryName = t(`mysteries.${todaysMystery}`);
  const todayMysteryDays = t(`days.${todaysMystery}`);
      ...
      <div className="min-w-0 flex flex-col min-h-screen">
        ...
        <div className="flex items-center justify-between px-11 py-[22px] border-b border-line sticky top-0 bg-ink/75 backdrop-blur-[14px] z-10 shrink-0">
          <div className="flex items-center gap-[18px] text-muted font-ui text-[13px]">
            <span className="font-display font-medium text-[18px] text-bone">
              {dateStr}
            </span>
        ...
        <div className="px-14 pb-20 max-w-[1280px] mx-auto w-full pt-0">
          ...
          <section className="grid grid-cols-[1.1fr_1fr] gap-[72px] items-center py-9 pb-14">
            <div>
              ...
              <div className="flex items-center gap-[18px]">
                <Link
                  href={`/${locale}/prayer/${todaysMystery}`}
                ...
                <Link
                  href={`/${locale}/prayer/${todaysMystery}?silent=1`}
            ...
            <BeadViz
              mysteryName={todayMysteryName}
              mysteryDay={todayMysteryDays}
            />
          </section>
          ...
          <div className="mt-20 pt-7 border-t border-line flex items-center justify-between text-muted-2 font-display italic text-[14px]">
            <span>Rosarium Today · 2026</span>
            <span>{t("footerAmdg")}</span>
          </div>
        </div>
      </div>
